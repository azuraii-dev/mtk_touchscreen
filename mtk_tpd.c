...
extern int synaptics_rmi4_init(void);
extern void synaptics_rmi4_exit(void);
#endif
#ifdef CONFIG_TOUCHSCREEN_Q20_TOUCH
extern int s2716b_ts_module_init(void);
extern void s2716b_ts_module_exit(void);
#endif
/*add by hodaone begin*/
#define PROC_NAME	"hodafone_tp"
static struct proc_dir_entry *tp_proc_entry;
/*add by hodaone end*/
int tp_compatible_flag=0;//add for hodafone
...
static struct notifier_block tpd_disp_notifier;
/* use disp_notifier */
#endif
...
#if IS_ENABLED(CONFIG_DRM_MEDIATEK)
int  tpd_disp_notifier_callback(
...
int tpd_driver_add(struct tpd_driver_t *tpd_drv)
{
	...
	if (strcmp(tpd_drv->tpd_device_name, "generic") == 0) {
		...
		tpd_driver_list[0].get_tp_firmware_version = tpd_drv->get_tp_firmware_version;
	...
	for (i = 1; i < TP_DRV_MAX_COUNT; i++) {
		...
		if (tpd_driver_list[i].tpd_device_name == NULL) {
			...
			tpd_driver_list[i].get_tp_firmware_version = tpd_drv->get_tp_firmware_version;
...
{
	if (g_tpd_drv->get_tp_firmware_version != NULL) {
		seq_printf(m, "%s\n%s\n", g_tpd_drv->tpd_device_name,g_tpd_drv->get_tp_firmware_version());
	} else {
		seq_printf(m, "%s\n%s\n", g_tpd_drv->tpd_device_name,"firmware version:not support");
	}
	
	return 0;
};

static int proc_hodafone_tp_open(struct inode *inode, struct file *file)
{
	return single_open(file, tp_debug_read, NULL);
};

static const struct proc_ops hodafone_tpd_fops = {
	.proc_read = seq_read,
	.proc_open = proc_hodafone_tp_open,
};
...
/*for some projects, we do not use mtk framebuffer*/
...
#if IS_ENABLED(CONFIG_DRM_MEDIATEK)
	tpd_disp_notifier.notifier_call = tpd_disp_notifier_callback;
	if (mtk_disp_notifier_register("mtk-touch", &tpd_disp_notifier))
		TPD_DMESG("Failed to register disp notifier client:%d");
#endif
#if IS_ENABLED(CONFIG_FB)
	/* use fb_notifier */
	...
	tp_proc_entry = proc_create(PROC_NAME, 0777, NULL,&hodafone_tpd_fops);
	/*add by hodafone end*/
	return 0;
}
static int tpd_remove(struct platform_device *pdev)
...
static int __init tpd_device_init(void)
{
	...
	tpd_log_init();
#ifdef CONFIG_TOUCHSCREEN_SYNAPTICS_DSX_I2C
	ret = synaptics_rmi4_init();
	if (ret) {
		pr_info("tpd : synaptics_rmi4_init init failed ret:%d\n", ret);
	}
#endif
#ifdef CONFIG_TOUCHSCREEN_Q20_TOUCH
	ret = s2716b_ts_module_init();
	if (ret) {
		pr_info("tpd : touch device init failed ret:%d\n", ret);
	}
#endif
...
#endif
#ifdef CONFIG_TOUCHSCREEN_Q20_TOUCH
	s2716b_ts_module_exit();
#endif
	/* input_unregister_device(tpd->dev); */
#if IS_ENABLED(CONFIG_DRM_MEDIATEK)
	mtk_disp_notifier_unregister(&tpd_disp_notifier);
#endif
#if IS_ENABLED(CONFIG_FB)
